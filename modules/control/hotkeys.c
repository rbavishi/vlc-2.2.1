...
static void SubSplitDisplay( intf_thread_t *, vout_thread_t *, input_thread_t * );
...
static int PutAction( intf_thread_t *p_intf, int i_action )
{
	...
	case ACTIONID_SUB_SPLIT:
            if( p_vout && vout_OSDEpg( p_vout, input_GetItem( p_input ) ) )
                SubSplitDisplay( p_intf, p_vout, p_input );
	    break;
    }
...
static void SubSplitDisplay( intf_thread_t *p_intf, vout_thread_t *p_vout,
                             input_thread_t *p_input )
{
    static char *prev_time=NULL;
    /* Extract Subtitle File */
    
    char psz_duration[MSTRTIME_MAX_SIZE];
    char psz_time[MSTRTIME_MAX_SIZE];
    vlc_value_t time, pos, state;
    mtime_t i_seconds;
    ...
    {
        state.i_int=0;
	var_Change( p_input, "sub-split", VLC_VAR_SETVALUE, &state, NULL ); 
        DisplayMessage( p_vout, "Subtitle-Split N/A" );
        return;
    }
    ...
    /*msg_Info( p_intf, _("Subtitle track: %s"),*/
		    /*list2.p_list->p_values[i_sel_index].psz_string );*/
    ...
    var_Get( p_input, "time", &time );
    var_Get( p_input, "sub-split", &state);
    i_seconds = time.i_time / 1000000;
    secstotimestr ( psz_time, i_seconds );
	...
	if( state.i_int == 0 )
	{
	    DisplayMessage( p_vout, "Split-Begin: %s", psz_time );
	    prev_time = strdup( psz_time );
	}
	else
	{
            if( p_input && var_GetInteger( p_input, "state" ) != PAUSE_S )
                var_SetInteger( p_input, "state", PAUSE_S );
	    DisplayMessage( p_vout, "Split-End: %s to %s", prev_time, psz_time );
	    msg_Info( p_intf, "%s", list2.p_list->p_values[i_sel_index].psz_string );
	    sprintf(command, "python /home/rbavishi/subtitle_extract.py '%s' %s %s", list2.p_list->p_values[i_sel_index].psz_string, prev_time, psz_time );
	    msg_Info( p_intf, "%s", command); 
	    system(command);
            var_SetInteger( p_input, "state", PLAYING_S );
	}

	state.i_int = ( state.i_int ^ 1 );
	var_Change( p_input, "sub-split", VLC_VAR_SETVALUE, &state, NULL ); 
    }
